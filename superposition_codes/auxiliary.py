...
def load_sweeps(
    folder, jobid, template="code_s_sweep_[0-9]+_{jobid}\.pkl", params_key="params"
):
    fls = os.listdir(folder)
    ...
    for ind in u.make_array_ind_iterator(pwr_emp.shape[:-1]):
        pwr_boot[ind] = u.bootstrap_list(pwr_emp[ind], np.mean, n=pwr_emp[ind].shape[0])
    ...
    for ind in u.make_array_ind_iterator(nus_emp.shape[:-1]):
        nus_boot[ind] = u.bootstrap_list(nus_emp[ind], np.mean, n=nus_emp[ind].shape[0])
    out = {
        "params": (pwrs, nus, dims),
        "args": data["args"],
